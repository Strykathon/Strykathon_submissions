...
from lib.datasets import CityscapesDataset, cityscapes_color_map
import os
import numpy as np
...
# add filenames into a python list
img_path_ds = tf.data.Dataset.list_files(f'{DATA_PATH}/leftImg8bit/{SPLIT}/*/*.png', shuffle=False)
img_name_list = []
for img_path in img_path_ds:
    split = tf.strings.split(img_path, os.path.sep).numpy()
    img_name = split[-1]
...
# Define the paths to save predicted masks
pred_path = f'predictions/{MODEL_NAME}/{SPLIT}/grayscale'
rgb_path = f'predictions/{MODEL_NAME}/{SPLIT}/rgb'
os.makedirs(pred_path, exist_ok=True)
os.makedirs(rgb_path, exist_ok=True)

# Loop through the dataset and save predicted masks
for dataset_elem, filename in zip(ds, img_name_list):
    ...
    print(f'Saving prediction for: {filename}')
    ...
    # Save RGB predictions in 'predictions' folder
    tf.keras.utils.save_img(f'{rgb_path}/{filename}', rgb_pred, data_format='channels_last', scale=False)
